...
extern "C" __global__ __aicore__ void sign_bf16_test(GM_ADDR x0, GM_ADDR output, GM_ADDR workspace, GM_ADDR gm_tiling_data);
  ...
  for (int i = 0; i < test_size; i++) {
    ...
    if (std::fabs(diff) > eps) {
      printf("Index %d: x=%f, y=%f, expect=%f, diff=%f\n",
             i, static_cast<float>(x[i]), static_cast<float>(y[i]), static_cast<float>(expect[i]), diff);
      diff_count++;
    }
  }
...
INSTANTIATE_TEST_SUITE_P(CalcWithDifferentShape, E2E_SignBf16_Code,

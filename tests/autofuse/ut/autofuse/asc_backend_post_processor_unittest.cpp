...
std::shared_ptr<AscGraph> ThreeScalarTwoSameValueDtypeAscGraph(ge::AscGraph &graph) {
  ...
  *x_out2.y.strides = {};
  ...
  // 构造图: x1 -> Load -> Brc -> Gt(B8) -> Store
  //         x2 -> Load -> Brc -> Gt
  // Gt输出DT_UINT8，后面直接接Store，broadcast不应移过Gt
  // 构造Compare图，三根轴(a,b,c)+中间轴(b)广播
  // with_cast=false: x1/x2 -> Load -> Brc -> Gt(UINT8) -> Store -> Output
  // with_cast=true:  x1/x2 -> Load -> Brc -> Gt(UINT8) -> Cast(UINT8->f32) -> Store -> Output
  std::shared_ptr<AscGraph> BroadcastBackwardCompareAscGraph(ge::AscGraph &graph, bool with_cast) {
    auto ONE = Symbol(1);
    auto ZERO = Symbol(0);
    const Expression A = graph.CreateSizeVar("A");
    const Expression B = graph.CreateSizeVar("B");
    ...
    std::vector<int64_t> abc = {a.id, b.id, c.id};
    std::vector<Expression> pre_brc_reps = {A, ONE, C};
    std::vector<Expression> pre_brc_strs = {C, ZERO, ONE};
    std::vector<Expression> post_brc_reps = {A, B, C};
    std::vector<Expression> post_brc_strs = {B * C, C, ONE};

    ge::ascir_op::Data x1("x1", graph);
    x1.attr.sched.axis = abc;
    x1.attr.sched.loop_axis = c.id;
    x1.y.dtype = DT_FLOAT;
    *x1.y.axis = abc;
    *x1.y.repeats = pre_brc_reps;
    *x1.y.strides = pre_brc_strs;

    ge::ascir_op::Load x1Local("x1Local");
    x1Local.x = x1.y;
    x1Local.attr.sched.axis = abc;
    x1Local.y.dtype = DT_FLOAT;
    *x1Local.y.axis = abc;
    *x1Local.y.repeats = pre_brc_reps;
    *x1Local.y.strides = pre_brc_strs;

    ge::ascir_op::Broadcast x1Brc("x1Brc");
    x1Brc.x = x1Local.y;
    x1Brc.attr.sched.axis = abc;
    x1Brc.y.dtype = DT_FLOAT;
    *x1Brc.y.axis = abc;
    *x1Brc.y.repeats = post_brc_reps;
    *x1Brc.y.strides = post_brc_strs;

    ge::ascir_op::Data x2("x2", graph);
    x2.attr.sched.axis = abc;
    x2.attr.sched.loop_axis = c.id;
    x2.y.dtype = DT_FLOAT;
    *x2.y.axis = abc;
    *x2.y.repeats = pre_brc_reps;
    *x2.y.strides = pre_brc_strs;

    ge::ascir_op::Load x2Local("x2Local");
    x2Local.x = x2.y;
    x2Local.attr.sched.axis = abc;
    x2Local.y.dtype = DT_FLOAT;
    *x2Local.y.axis = abc;
    *x2Local.y.repeats = pre_brc_reps;
    *x2Local.y.strides = pre_brc_strs;

    ge::ascir_op::Broadcast x2Brc("x2Brc");
    x2Brc.x = x2Local.y;
    x2Brc.attr.sched.axis = abc;
    x2Brc.y.dtype = DT_FLOAT;
    *x2Brc.y.axis = abc;
    *x2Brc.y.repeats = post_brc_reps;
    *x2Brc.y.strides = post_brc_strs;

    ge::ascir_op::Gt gt("compare_gt");
    gt.x1 = x1Brc.y;
    gt.x2 = x2Brc.y;
    gt.attr.sched.axis = abc;
    gt.y.dtype = DT_UINT8;
    *gt.y.axis = abc;
    *gt.y.repeats = post_brc_reps;
    *gt.y.strides = post_brc_strs;

    ge::ascir_op::Cast gt_cast("gt_cast");
    if (with_cast) {
      gt_cast.x = gt.y;
      gt_cast.attr.sched.axis = abc;
      gt_cast.y.dtype = DT_FLOAT;
      *gt_cast.y.axis = abc;
      *gt_cast.y.repeats = post_brc_reps;
      *gt_cast.y.strides = post_brc_strs;
    }
    ...
    if (with_cast) {
      ...
      x_store.x = gt.y;
      x_store.y.dtype = DT_UINT8;
    }
    x_store.attr.sched.axis = abc;
    x_store.attr.sched.loop_axis = c.id;
    *x_store.y.axis = abc;
    *x_store.y.repeats = post_brc_reps;
    *x_store.y.strides = post_brc_strs;

    ge::ascir_op::Output x_out("x_out");
    x_out.x = x_store.y;
    x_out.attr.sched.axis = abc;
    x_out.attr.sched.loop_axis = c.id;
    x_out.y.dtype = x_store.y.dtype;
    *x_out.y.axis = abc;
    *x_out.y.repeats = post_brc_reps;
    *x_out.y.strides = post_brc_strs;
...
// Broadcast后移判断：Gt输出B8且后面是Store(非Cast)，broadcast不后移
TEST_F(AscBackendPostProcessorTest, BroadcastBackward_CompareAscGraph) {
  ComputeGraphPtr compute_graph = BuildGraph1("AscBackend");
  EXPECT_EQ(compute_graph->GetAllNodesSize(), 5);
  ...
  attr1->SetAscGraph(BroadcastBackwardCompareAscGraph(add_graph1, false));
  AscBackendPostProcessor post_processor;
  EXPECT_EQ(asc_adapt::GeFallback(compute_graph), SUCCESS);
  auto shape_env_attr = compute_graph->GetOrCreateAttrsGroup<ShapeEnvAttr>();
  ASSERT_NE(shape_env_attr, nullptr);
  ...
  for (auto node : AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    if (node->GetName() == "compare_gt") {
      // 校验Broadcast节点没有后移到Gt后，Gt的前驱仍为Broadcast
      NodePtr pre_node;
...
// Broadcast后移判断：Gt输出B8但后面接Cast转f32，broadcast应移过Gt和Cast
TEST_F(AscBackendPostProcessorTest, BroadcastBackward_CompareAscGraph_Ok) {
  ...
  ge::AscGraph add_graph1("compare");
  attr1->SetAscGraph(BroadcastBackwardCompareAscGraph(add_graph1, true));
  AscBackendPostProcessor post_processor;
  EXPECT_EQ(asc_adapt::GeFallback(compute_graph), SUCCESS);
  auto shape_env_attr = compute_graph->GetOrCreateAttrsGroup<ShapeEnvAttr>();
  ASSERT_NE(shape_env_attr, nullptr);
  ...
  for (auto node : AscGraphUtils::GetComputeGraph(*(attr1->GetAscGraph()))->GetDirectNode()) {
    ...
    if (node->GetName() == "compare_gt") {
      // 校验Broadcast已后移，Gt的前驱应为Load（不再是Broadcast）
      NodePtr pre_node;
      asc_adapt::GetPeerOutNode(node, pre_node, 0);
      ASSERT_EQ(pre_node->GetType(), "Load");
    }
  }
  // broadcast移过Gt和Cast后，存在1个Broadcast节点（在Cast之后）
  ASSERT_EQ(broadcast_cnt, 1);
}

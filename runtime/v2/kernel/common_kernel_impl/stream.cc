...
ge::graphStatus SyncStream(KernelContext *context) {
  auto stream = context->GetInputValue<aclrtStream>(0);
 ...
 *
 * input 0: TypedContinuousVector<aclrtStream>
 * input 1: stream_num
 * output 0: aclrtStream
...
ge::graphStatus SplitRtStreams(KernelContext *context) {
  auto logic_streams_to_rts_stream =
      context->GetInputPointer<TypedContinuousVector<aclrtStream>>(EnToIdx(SplitRtStreamsInput::kStreams));
  ...
  for (size_t i = 0U; i < static_cast<size_t>(*stream_num); ++i) {
    auto rts_stream = context->GetOutputPointer<aclrtStream>(i);
